...
//----- Variaveis para memória compartilhada ---------
...
//  	CvCapture* captura1 = cvCaptureFromCAM( 1 );
	...
	{
		printf( "Failed to open USB0Dynamixel!\n" );
		//printf( "Press Enter key to terminate...\n" );
		//getchar();
		//return 0;
	}
	else
		printf( "Succeed to open USB0Dynamixel!\n" );
    ...
    else
    {
        printf("USB wrong");
        dxl_terminate();
	    // ---- Open USB2Dynamixel again -------------------
	   if( dxl_initialize(deviceIndex+1, baudnum) == 0 )
	   {
            printf( "Failed to open USB1Dynamixel!\n" );
            printf( "Press Enter key to terminate...\n" );
            getchar();
            return 0;
        }
        else
            printf( "Succeed to open USB1Dynamixel!\n" );
...
//-----------------------------------------------------------------------------
	...
	// O Thresholding é o metodo mais simples de segmentação de imagens.
        ...
        //cvSmooth( segmentada, segmentada, CV_GAUSSIAN, 9, 9 );
        CvSeq* circulos = cvHoughCircles(segmentada, memoria, CV_HOUGH_GRADIENT, 2, segmentada->height/4, 100, 50, 10, 100);// 200 100 10 400
                                                                                                                            //vilão 100 50 10 400
        std::string str;
        //static float mx = 0;
        //static float my = 0;
...
//	cvCircle( Quadro, cvPoint(cvRound(p[0]),cvRound(p[1])), cvRound(p[2]), CV_RGB(255,255,255), 2, 8, 0 );
        ...
        // Calculate the moments to estimate the position of the ball
           ...
           // const char * posx = posix.c_str();
            ...
            char posx[10];
            sprintf(posx, "DifX %d", (posX - RESOLUCAO_X/2));
            cvPutText (Quadro, posx ,cvPoint(100,400), &font, cvScalar(255,255,0));
            ...
            char rd[10];
            sprintf(rd, "Raio: %f", p[2]);
            cvPutText (Quadro, rd ,cvPoint(300,300), &font, cvScalar(255,255,0));
        ...
        if(lost_ball>20) //verifica se depois de 30 frames a bola está perdida no campo
        {
            ...
            RETORNO_VISAO = 0; //Retorna que a bola não esta em seu campo de visão
            //DISTANCIA_BOLA = 0; // distancia 0 porque precisa ficar parado procurando
        ...
        else
        {
           ...
           // {
            ...
            //while(*comunica!=0);
            //if(*comunica)
                if (head_follow_ball(posX, posY, &head_move1, &head_move2) == 1)
...
//	cvShowImage( "Video2", Quadro2 ); // Stream Original com a bola detectada
	...
	// Isso é feito removendo os bits de maior significancia usando o operador AND
        ...
        if( (cvWaitKey(10) & 255) == 27 || saida > 15 || cont_search_ball > 9 )
        {
...
//      cvReleaseCapture( &captura1 );
        ...
        //cvDestroyWindow( "Sistema de cores RGB - HSV" );
            if (cont_search_ball > 9)
            {
                ...
                //dxl_write_word(1, P_GOAL_POSITION_L, 304);
                //return 2;
                RETORNO_VISAO = 2; //Retorna que a bola esta pedida
                cont_search_ball = 0;
            }
            //else
                //return 1;
            if(saida>15)
            {
                SERVO1_POSITION = dxl_read_word( 1, P_PRESENT_POSITION_L);
                SERVO2_POSITION = dxl_read_word( 2, P_PRESENT_POSITION_L);
                DISTANCIA_BOLA = distancia_bola()*1000; // Retorna a distância do robô para a bola
                ANGULO_BOLA = (594-dxl_read_word( 2, P_PRESENT_POSITION_L)); // Angulo entre bola robo
                RETORNO_VISAO = 1; //Retorna que a bola foi encontrada
                saida=0;
                usleep(2000);
            }
        }
...
//------ Função que faz a busca da bola quando ela está em uma posição desconhecida---
void search_ball(bool inicio)
{
    // Posição inicial da cabeça {304, 594} //01 , 02, cabeça
static unsigned int varredura=0;
    ...
    usleep(1000); //Dá um tempo para aliviar o processamento após o comando
